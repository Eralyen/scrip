...
`SCRIP` uses your wallet credentials and 1337coin username to work. User safety is very important to me, and I have implemented [strict security features.](#security-features)
...
After entering your credentials, you should see the main screen, which is updated every few seconds. You can then press `D` to start trading, `WASD` to edit your trade, `X` to cancel, or `E` to confirm the trade. Alternatively, press `A` to auto trade.

A trade in angle brackets like \<+¢1 -$137\> is not confirmed yet.
